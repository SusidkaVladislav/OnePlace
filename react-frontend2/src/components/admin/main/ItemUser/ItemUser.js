...
import ArrowDown from '../ItemMain/svg/ArrowDown';
...
const ItemUser =({size})=>{
  ...
  const options = ['Прізвище', 'Ім"я', 'Телефон', 'E-mail'];
  ...
  const filteredTableData = users.filter((user) => {
    ...
    const userFieldValue = user[selectedItem]?.toLowerCase()||'';
  ...
  }, [currentPage, inputValue, selectedItem,users]);
        ...
        <div className='user-main' style={{ display: isUserDivVisible ? 'none' : 'block' }}>
          <div className="search-div">
            ...
            <div className='search-radio-div'>
              ...
              <div className={`user-dropdown-header ${isOpen ? 'open' : ''}`} onClick={handleToggleDropdown}>
          
                <label className='user-main-select-item'>{selectedItem}</label>
                <label className='user-main-arrow-down'><ArrowDown/></label>
                {isOpen && (
                  <div className="user-dropdown-list">
                    {options.map((item, index) => (
                      <label
                        key={index}
                        className={`user-dropdown-item ${selectedItem === item ? 'selected' : ''}`}
